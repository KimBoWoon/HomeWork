...
public class CirclesComponent extends JComponent
{
	...
	public CirclesComponent()
	{
		...
		for (int i = 1; i <= NCIRCLES; i++)
		{
			...
			int x = ran.nextInt(), y = ran.nextInt(), r = ran.nextInt();
			
			Ellipse2D.Double circle1 = new Ellipse2D.Double(x-r, y-r, r*2, r*2);
			Ellipse2D.Double circle2 = new Ellipse2D.Double(x-r, y-r, r*2, r*2);
...
@return true if c1 and c2 intersect
	...
	public void paintComponent(Graphics g)
	{
		...
		for (int i = 0; i < circles.size(); i++) // iterate through every circle in the list
			g2.draw(circles.get(i)); // draw the circle
	}
